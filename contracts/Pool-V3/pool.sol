...
contract PoolBRSTv3 is Ownable{
  ...
  TRC20_Interface Principal_Contract = TRC20_Interface(0x389ccc30de1d311738Dffd3F60D4fD6188970F45);
  ...
  function RATE() public view returns (uint){
    ...
    return (TRON_BALANCE().mul(10**Principal_Contract.decimals())).div( Principal_Contract.totalSupply() );
  ...
  function ChangeToken(address _tokenTRC20) public onlyOwner returns (bool){
    ...
    Principal_Contract = TRC20_Interface(_tokenTRC20);
  ...
  function newOwnerPrincipalToken(address _newowner) public onlyOwner returns (bool){

    Principal_Contract.transferOwnership(_newowner);

    return true;

  }

  function solicitudVenta(uint256 _value) public returns (uint256){

    if( Principal_Contract.allowance(msg.sender, address(this)) < _value || Principal_Contract.balanceOf(msg.sender) < _value)revert();

    uint pago = _value.mul(RATE()).div(10 ** Principal_Contract.decimals());
    
    if( !Principal_Contract.transferFrom(msg.sender, address(this), _value) )revert();
  ...
  function retirar(uint256 _id) public {
    ...
    Principal_Contract.redeem(usuario.brst[_id]);
  ...
  function completarSolicitud(uint256 _index) public payable returns (bool){
    ...
    Principal_Contract.transfer(msg.sender, usuario.brst[_id]);
  ...
  function buyToken() public payable returns (uint) {
    ...
    _value = (_value.mul( 10 ** Principal_Contract.decimals() )).div(RATE());
    TRON_WALLET_BALANCE += msg.value;

    Principal_Contract.issue(_value);

    Principal_Contract.transfer(msg.sender,_value);
  ...
  function retiroBalance(uint _value) public onlyOwner returns(uint){
  ...
  function depositoBalance(uint _value) public onlyOwner returns(uint){
  ...
  function crearTokenPrincipal(uint _value) public onlyOwner returns(bool){
    Principal_Contract.issue(_value);
    Principal_Contract.transfer(owner, _value);

    return true;
      
  }

  function quemarTokenPrincipal(uint _value) public onlyOwner returns(bool, uint256){

    if( Principal_Contract.allowance(msg.sender, address(this)) < _value || 
    Principal_Contract.balanceOf(msg.sender) < _value ||
    !Principal_Contract.transferFrom(msg.sender, address(this), _value))revert();
      
    Principal_Contract.redeem(_value);

    return (true,_value);
      
  }

  function retirarTodoTokenPrincipal() public onlyOwner returns (uint256){

    uint256 valor = Principal_Contract.balanceOf(address(this));

    Principal_Contract.transfer(owner, valor);

    return valor;
  }

  function retirarTokenPrincipal(uint _value) public onlyOwner returns (uint256) {

    if ( Principal_Contract.balanceOf(address(this)) < _value)revert();
  ...
  function retirarTodoTokenOTRO() public onlyOwner returns (uint256){
  ...
  function retirarTRX() public onlyOwner returns (uint256){
